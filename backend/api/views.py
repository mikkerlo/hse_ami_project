...
def deadline_view(request, deadline_id):
    ...
    if request.method == 'GET':
        ...
        if not _can_modify_deadline(request.student, deadline):
            return _STATUS_FORBIDDEN, 'Only group moderator or creator can edit this deadline'
        data = json.loads(request.body)
...
def deadline_new(request):
    ...
    if not _is_group_moderator(request.student, data['group_id']):
        return _STATUS_FORBIDDEN, 'Only moderator or creator can add deadlines to the group'
