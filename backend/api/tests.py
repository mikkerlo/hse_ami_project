...
class APITestCase(TestCase):
    ...
    def test_get_student_ok(self):
        ...
        response = c.get(reverse('api_students_id', kwargs={'id': id}))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.content)['result']
    ...
    def test_patch_student_ok(self):
        ...
        # Updating user name
        response = c.patch(reverse('api_students_id', kwargs={'id': id}),
    ...
    def test_get_deadlines(self):
        c = Client()
        id = self.students[0]['id']
        response = c.get(reverse('api_students_deadlines', kwargs={'id': id}))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.content)['result']
    ...
    def test_get_groups_one_groups(self):
        c = Client()
        id = self.students[0]['id']
        response = c.get(reverse('api_students_groups', kwargs={'id': id}))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.content)['result']
